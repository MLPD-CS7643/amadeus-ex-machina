...
def convert_audio(input_file, output_folder, output_format, mp3_bitrate):
...
def batch_convert(folder_path, input_ext, output_folder, output_format, mp3_bitrate=64, n_jobs=1):
    ...
    with tqdm_joblib(tqdm(desc="Converting files...", total=len(files))) as progress_bar:
        Parallel(n_jobs=n_jobs)(delayed(process_file)(file, output_folder, output_format, mp3_bitrate) for file in files)
